...
import { DogWrap, Title, BtnWrap, Btn, TextWrap, Name, Info } from './dog';
...
const Dog = () => {
  const [TitleState, setTitleState] = useState(catArray[0].name);
  ...
  const handleBtn = num => {
    const cat = catArray[num];
    setTitleState(cat.name);
    setTextState(cat.text);
    setButtonId(num);
  };
